...
`general_pickup` 支持纯空间方位指令（如"拿起最左侧的物体"），完整链路包括：

- **Rigid 加载**：从预生成的 Eval Layout JSON 把桌面刚体还原为可交互对象
- **目标解析**：指令 → 空间方位词 → 目标物体（如 leftmost = min(x)）
- **成功判定**：目标物体被抬起 ≥10cm 即成功
- **Cheat 验证机制**：无真实策略时，`cheat_lift_step` 在指定步随机抬升一个桌面刚体，用于端到端验证 prompt 生成、目标解析与成功判定（不强制成功）
